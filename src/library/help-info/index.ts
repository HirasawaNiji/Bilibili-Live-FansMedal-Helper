    ...
    LiveTasks: {
      medalTasks: {
        ...
        light: {
          ...
          message: () =>
            h('p', [
              ...
              h('div', '本定制版只使用点赞点亮，不会自动发送弹幕。'),
              h('ul', [
                h('li', '主播已经开播：按B站任务要求点赞，并在执行后确认勋章确实恢复点亮。'),
                h('li', '主播尚未开播：进入等待队列，每约2分钟检查一次，开播后立即点赞。'),
              ]),
              h('div', [
                h('strong', '注意：'),
                h('ul', [
                  h('li', '等待未开播主播时不会阻塞独立的点赞和观看直播任务。'),
                  h(
                    'li',
                    '储蓄亲密度达到90后会显示在直播任务页顶部的“以下主播储蓄亲密度即将溢出”提醒中，并显示主播当前开播状态。',
                  ),
                ]),
              ]),
            ]),
          ...
          message: () =>
            h('p', [
              ...
              h('ul', [
                ...
                h('li', '只会在执行前确认主播正在直播后发送观看心跳。'),
                h('li', '每完成一轮观看都会读取B站任务进度，未计入时停止该房间。'),
              ]),
              h('div', [
                h('strong', '注意：'),
                h(
        ...
        watchWaitUntilLiving: {
          ...
          message: () =>
            h('p', [
              h('div', '开启后，当前未开播的名单内主播会进入等待队列。'),
              h('div', '脚本每隔约2分钟检查一次；主播开播后优先执行其观看任务。'),
              h('div', '关闭后，只处理脚本开始运行时已经开播的主播。'),
            ]),
        },
      },
    },
